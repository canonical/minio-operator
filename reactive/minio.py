...
def configure_minio(gipup):
    if not Path("/run/password").exists():
        Path("/run/password").write_text(
            "".join(choices(ascii_uppercase + digits, k=30))
        )

    gipup.publish_info(
        port=hookenv.config("port"),
        ip=hookenv.application_name(),
        user=hookenv.config("access-key"),
...
def start_charm():
    ...
    if not Path("/run/password").exists():
        if config.get("secret-key"):
            secret_key = config["secret-key"]
        else:
            secret_key = "".join(choices(ascii_uppercase + digits, k=30))

        Path("/run/password").write_text(secret_key)
